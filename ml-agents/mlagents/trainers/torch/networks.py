...
class ObservationEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.rsa is not None:
            ...
            total_enc_size = 2 * self.ATTENTION_EMBEDDING_SIZE
        else:
    ...
    def update_normalization(self, buffer: AgentBuffer) -> None:
        ...
        for vec_input, enc in zip(obs, self.processors):
            if isinstance(enc, VectorInput):
                enc.update_normalization(torch.as_tensor(np.array(vec_input)))
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        if len(var_len_processor_inputs) > 0 and self.rsa is not None:
            ...
            if not input_exist:
                ...
                encoded_self = torch.cat([processed_self, attention_embedding], dim=1)
