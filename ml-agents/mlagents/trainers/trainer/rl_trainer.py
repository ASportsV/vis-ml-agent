...
class RLTrainer(Trainer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.collected_test_rewards: Dict[str, Dict[str, int]] = {
            "environment": defaultdict(lambda: 0)
        }
    ...
    def _write_summary(self, step: int) -> None:
        ...
        # np_obs = ObsUtil.from_buffer(self.update_buffer, len(self.policy.behavior_spec.observation_specs))
        # tensor_obs = [ModelUtils.list_to_tensor(obs) for obs in np_obs]
        # writer = SummaryWriter('results')
        # writer.add_graph(list(self.policy.get_modules().values())[0], [tensor_obs])
        # writer.close()
    ...
    def _process_test_trajectory(self, test_trajectory: Trajectory) -> None:
        agent_id = test_trajectory.agent_id  # All the agents should have the same ID

        agent_buffer_trajectory = test_trajectory.to_agentbuffer()
        # should use another
        self.collected_test_rewards["environment"][agent_id] += np.sum(
            agent_buffer_trajectory[BufferKey.ENVIRONMENT_REWARDS]
        )
        # If this was a terminal trajectory, append stats and reset reward collection
        if test_trajectory.done_reached:
            for name, rewards in self.collected_test_rewards.items():
                if name == "environment":
                    self.stats_reporter.add_stat(
                        "Environment/Cumulative Test Reward",
                        rewards.get(agent_id, 0),
                        aggregation=StatsAggregationMethod.HISTOGRAM,
                    )
                    rewards[agent_id] = 0
        ...
        with hierarchical_timer("process_trajectory"):
            for traj_queue in self.trajectory_queues:
                ...
                if self.threaded and not _queried:
                    ...
                    time.sleep(0.0001)
            for test_traj_queue in self.test_trajectory_queues:
                _queried = False
                for _ in range(test_traj_queue.qsize()):
                    _queried = True
                    try:
                        t = test_traj_queue.get_nowait()
                        self._process_test_trajectory(t)
                    except AgentManagerQueue.Empty:
                        break
        if self.should_still_train:
            if self._is_ready_update():
                with hierarchical_timer("_update_policy"):
                    if self._update_policy():
                        ...
                        # @TEST push to a test policy queue
                        for q in self.test_policy_queues:
                            q.put(self.get_policy(q.behavior_id))