...
def run_training(run_seed: int, options: RunOptions, num_areas: int) -> None:
    ...
    with hierarchical_timer("run_training.setup"):
        ...
        test_env_factory = create_environment_factory(
            env_settings.env_path,
            engine_settings.no_graphics,
            run_seed,
            num_areas,
        ...
        test_env_manager = SubprocessEnvManager(test_env_factory, options, 1) if env_settings.env_path is not None else None
    ...
    # Begin training
    try:
        tc.start_learning(env_manager, test_env_manager)
    finally:
        env_manager.close()
        if test_env_manager is not None:
            test_env_manager.close()
