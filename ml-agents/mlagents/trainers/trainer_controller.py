...
class TrainerController:
    ...
    @timed
    def _reset_env(self, env_manager: EnvManager, test_env_manager: EnvManager) -> None:
        ...
        if test_env_manager is not None:
            test_new_config = new_config.copy()
            test_new_config['_test_mode'] = ConstantSettings(seed=7797, value=1.0)
            test_env_manager.reset(config=test_new_config)
        # Register any new behavior ids that were generated on the reset.
        self._register_new_behaviors(env_manager, env_manager.first_step_infos, test_env_manager)
    ...
    def _create_trainer_and_manager(
        self, env_manager: EnvManager, name_behavior_id: str, test_env_manager: EnvManager
    ) -> None:
        ...
        if test_env_manager is not None:
            ...
            test_env_manager.set_agent_manager(name_behavior_id, test_agent_manager)
            test_env_manager.set_policy(name_behavior_id, policy)
            trainer.subscribe_test_trajectory_queue(test_agent_manager.trajectory_queue)
            trainer.publish_test_policy_queue(test_agent_manager.policy_queue)
    ...
    def _create_trainers_and_managers(
        self, env_manager: EnvManager, behavior_ids: Set[str], test_env_manager: EnvManager,
    ) -> None:
        for behavior_id in behavior_ids:
            self._create_trainer_and_manager(env_manager, behavior_id, test_env_manager)

    @timed
    def start_learning(self, env_manager: EnvManager, test_env_manager: EnvManager) -> None:
        self._create_output_path(self.output_path)
        try:
            # Initial reset
            self._reset_env(env_manager, test_env_manager)
            self.param_manager.log_current_lesson()
            while self._not_done_training():
                n_steps = self.advance(env_manager, test_env_manager)
                for _ in range(n_steps):
                    self.reset_env_if_ready(env_manager, test_env_manager)
    ...
    def reset_env_if_ready(self, env: EnvManager, test_env: EnvManager) -> None:
        ...
        if param_must_reset or ghost_controller_reset:
            self._reset_env(env, test_env)  # This reset also sends the new config to env
            self.end_trainer_episodes()
        elif updated:
            env.set_env_parameters(self.param_manager.get_current_samplers())
            test_env.set_env_parameters(self.param_manager.get_current_samplers())

    @timed
    def advance(self, env_manager: EnvManager, test_env_manager: EnvManager) -> int:
        # Get steps
        with hierarchical_timer("env_step"):
            new_step_infos = env_manager.get_steps()
            self._register_new_behaviors(env_manager, new_step_infos, test_env_manager)
            num_steps = env_manager.process_steps(new_step_infos)

        if test_env_manager is not None:
            # test env step
            with hierarchical_timer("test_env_step"):
                new_step_infos = test_env_manager.get_steps()
                test_env_manager.process_steps(new_step_infos)
    ...
    def _register_new_behaviors(
        self, env_manager: EnvManager, step_infos: List[EnvironmentStep], test_env_manager: EnvManager,
    ) -> None:
        """
        ...
        self._create_trainers_and_managers(env_manager, new_behavior_ids, test_env_manager)
