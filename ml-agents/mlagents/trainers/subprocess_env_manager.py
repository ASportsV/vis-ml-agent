    ...
    try:
        ...
        while True:
            ...
            if req.cmd == EnvironmentCommand.STEP:
                ...
                _test_end = env_stats['_test/_test_end']
                _test_done = False
                if len(_test_end) > 0 and _test_end[0][0] == 1:
                    _test_done = True
                ...
                step_queue.put(
                    EnvironmentResponse(
                        EnvironmentCommand.STEP if not _test_done else EnvironmentCommand.TEST_DONE, worker_id, step_response
                    )
                )
                reset_timers()
...
class SubprocessEnvManager(EnvManager):
    ...
    def _queue_steps(self) -> None:
        '''Generate actions based on the latest policy
        '''
            ...
            try:
                while True:
                    ...
                    if step.cmd == EnvironmentCommand.ENV_EXITED:
                        ...
                        self.env_workers[step.worker_id].waiting = False
                        if step.cmd == EnvironmentCommand.TEST_DONE:
